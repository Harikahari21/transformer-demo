...
import { quarterlyArray } from "@/utils/array";

const QuarterlyCompliance = () => {
  const quarterlyPageArray = [
    {
      pageName: "Quarterly Compliance",
      pagePath: ``,
    },
  ];
  return (
    <PublicLayout>
      <Banner title="Quarterly Compliance" pathArr={quarterlyPageArray} />
        ...
        <div className="max-w-full overflow-x-scroll scroll-bar-none">
          <div className=" flex flex-col border rounded-sm border-[#B9B9B9]/40 w-[45rem] md:w-full">
            ...
            <div className="w-full flex flex-col bg-white shadow-shadow-primary rounded-b-sm">
              {quarterlyArray?.map((item, i) => (
                <div
                  className={`grid grid-cols-5 divide-x text-sm  ${
                    i !== quarterlyArray.length - 1 ? " border-b" : ""
                  }`}
                  key={i}
                >
